...
for k, v in qwen_type_to_module_mapping.items():
    qwen_lm_type_to_module_mapping[k] = (f"model.{v[0]}", ) + v[1:]
...
for k, v in qwen_type_to_module_mapping.items():
    qwen_classifier_type_to_module_mapping[k] = (f"model.{v[0]}", ) + v[1:]
...
def create_qwen(
    name="Qwen/Qwen2-0.5B", cache_dir=None, dtype=torch.bfloat16, config=None,
    revision='main'
):
    """Creates a qwen Causal LM model, config, and tokenizer from the given name and revision"""
    if config is None:
        ...
        qwen = AutoModelForCausalLM(config, cache_dir=cache_dir, revision=revision)
        tokenizer = AutoTokenizer.from_pretrained(name, cache_dir=cache_dir)
    print("loaded model")
    return config, tokenizer, qwen
